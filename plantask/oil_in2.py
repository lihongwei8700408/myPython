...
def getState(mid):
	...
	try:
		...
		return ''
sqlm = 'SELECT Id,MemberId,RegisteTime FROM cl_sharemoney_record'
try:
	# 执行SQL语句
	zn = cur.execute(sqlm)
	result = cur.fetchall()
	...
	for row in result:
		...
		id = row['Id']
		sub = getState(mid)
		sql = 'INSERT INTO oil_in(MemberId,AddTime,Money,Type,RelationId,IsSubscribe) VALUES ('+str(mid)+','+str(t)+',290,4,'+str(id)+','+str(sub)+')'
		print sql
		try:
			n = cur.execute(sql)
			print n
