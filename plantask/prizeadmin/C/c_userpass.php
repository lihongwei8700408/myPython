<?php
	function actionV_index(){
		global $_USER;
		$layout_content = './V/v_userpass.php';
		include './V/layout.php';
	}
	if(isset($_POST['submit'])){
		if(isset($_POST['oldpass'])&&$_POST['oldpass']!= $_USER->Password){
			echo '<script>alert("旧密码输入不正确");history.go(-1);</script>';
			exit;
		}
		...
		if($ret){
			...
			echo '<script>alert("操作失败");history.go(-2);</script>';
		}
	}
