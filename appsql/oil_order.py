...
try:
	...
	for row in result:
		...
		SellerOrderId = dealnone(row['order_id'])
		State = orderstate(row['order_state'],row['recharge_app_result'])
		JkType = jktype(row['tp'])
		sql = 'INSERT INTO oil_charge_recode(source_id,member_id,time,price,oil_money,money,integral,oil_card,seller_order_id,state,jk_type,source) VALUES ('+str(SourceId)+','+str(MemberId)+','+str(Time)+','+str(Price)+',0,'+str(Money)+','+str(Integral)+',"'+OilCard+'","'+SellerOrderId+'",'+str(State)+','+str(JkType)+',2)'
		n = cur.execute(sql)
		y += 1
		baifen = (y * 100) / AllNum
