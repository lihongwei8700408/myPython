...
try:
	...
	exit()
sql = 'SELECT member_id,tel FROM zz_member_oilcard where isdelete="no" and oil_tp="1" group by tel'
try:
	# 执行SQL语句
	y = 0
	...
	for i in list:
		sql = 'SELECT * FROM zz_member_oilcard where isdelete="no" and oil_tp="1" and member_id='+str(i)+' order by last_time desc limit 1'
		n = cur.execute(sql)
		result = cur.fetchone()
		id = result['id']
		...
		if have>0:
			...
			if h==0:
				sql = 'INSERT INTO oil_card(oil_card,member_id,is_show,oil_type,time,source_id) VALUES ("'+card+'",'+str(have)+',0,2,'+str(last_time)+','+str(id)+')'
				n = cur.execute(sql)
				y += 1
				baifen = (y * 100) / AllNum
