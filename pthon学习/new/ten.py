...
def curlget(orderid,key):
	...
	return r
sqlm = 'SELECT Id,Price,OilMoney,SellerOrderId,MemberId FROM oil_charge_recode where Id = 14029'
try:
	# 执行SQL语句
	zn = cur.execute(sqlm)
	print zn
	...
	for row in result:
		...
		if state == '1':
			...
			if price > jifen:
				...
				n = cur.execute(sql)
				sql1 = 'UPDATE cl_member SET OilMoney = OilMoney + '+str(jifen)+' where Id = '+str(mid)
				n = cur.execute(sql1)
				if n == 0:
					continue
