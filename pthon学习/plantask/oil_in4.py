...
def getState(mid):
	...
	try:
		...
		return ''
sqlm = 'SELECT Id,MemberId,Time,Money FROM cl_money_benefit'
try:
	# 执行SQL语句
	zn = cur.execute(sqlm)
	result = cur.fetchall()
	...
	for row in result:
		...
		m = int(row['Money']*100)
		print m
		sql = 'INSERT INTO oil_in(MemberId,AddTime,Money,Type,RelationId,IsSubscribe) VALUES ('+str(mid)+','+str(t)+','+str(m)+',5,'+str(id)+','+str(sub)+')'
		try:
			n = cur.execute(sql)
		except:
			continue	
