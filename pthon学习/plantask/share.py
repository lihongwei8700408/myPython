...
def getState(mid):
	...
	try:
		...
		return ''
sqlm = 'SELECT Id,MemberId,AddTime FROM oil_in where Type=4'
try:
	# 执行SQL语句
	zn = cur.execute(sqlm)
	result = cur.fetchall()
	...
	for row in result:
		...
		sub = getState(mid)
		print sub
		sql = 'INSERT INTO cl_sharemoney_record_copy(MemberId,RegisteTime,BondId,IsSubscribe) VALUES ('+str(mid)+','+str(t)+','+str(id)+','+str(sub)+')'
		try:
			n = cur.execute(sql)
		except:
			continue	
