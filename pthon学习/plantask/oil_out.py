...
def getState(mid):
	...
	try:
		...
		return ''
sqlm = 'SELECT Id,MemberId,UsePoint,LastTime,OilCard FROM cl_oil_order where UsePoint > 0 AND (OilPayState=3 OR OilPayState=1 OR (OilPayState=2 AND OrderState=5))'
try:
	# 执行SQL语句
	zn = cur.execute(sqlm)
	result = cur.fetchall()
	...
	for row in result:
		...
		oilcard = row['OilCard']
		sub = getState(mid)
		sql = 'INSERT INTO oil_out(MemberId,Time,Money,OilCard,Type,OrderId,IsSubscribe) VALUES ('+str(mid)+','+str(t)+','+str(oilmoney)+','+str(oilcard)+',1,'+str(oid)+','+str(sub)+')'
		try:
			n = cur.execute(sql)
		except:
			continue
